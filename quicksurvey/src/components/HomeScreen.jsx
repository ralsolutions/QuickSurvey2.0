...
import { C, ROLES, ST } from '../lib/constants.js';
import { dbGetAll, dbDelete, exportProject, importProjectFromFile } from '../lib/db.js';

// Lazy load the role switcher — only fetched when user opens the modal
const RoleSwitcher = lazy(() => import('./RoleSwitcher.jsx').then(m => ({ default: m.RoleSwitcher })));
...
export function HomeScreen({ onOpen, onCreate, user, onRoleSwitch, onSwitchUser, onLogout }) {
  ...
  const handleExport = async (e, p) => {
    e.stopPropagation();
    try { await exportProject(p); } catch (err) { alert('Export failed: ' + err.message); }
  };
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: C.bg }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', display: 'flex', justifyContent: 'center', padding: '32px 16px' }}>
        <div style={{ width: 560, maxWidth: '100%' }}>
          ...
          {list.map(p => {
            ...
            return (
              <div key={p.id}
                style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 10, padding: '14px 16px', display: 'flex', alignItems: 'center', cursor: 'pointer', marginBottom: 8, boxShadow: '0 1px 3px rgba(0,0,0,0.06)', transition: 'box-shadow 0.15s' }}
                onClick={() => onOpen(p)}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: C.textDim, marginTop: 3, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    <span>{p.elevations.length} elevation{p.elevations.length !== 1 ? 's' : ''}</span>
                    <span>·</span>
                    <span>{ptotal} pins</span>
      ...
      {showRole && (
        <Suspense fallback={null}>
          <RoleSwitcher user={user} onSwitch={r => { onRoleSwitch(r); setShowRole(false); }} onSwitchUser={u => { onSwitchUser(u); setShowRole(false); }} onLogout={onLogout} onClose={() => setShowRole(false)}/>
        </Suspense>
      )}
    </div>
  );
