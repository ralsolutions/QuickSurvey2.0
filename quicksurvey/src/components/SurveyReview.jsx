...
import { C, RT, HC, HAZARDS, ST } from '../lib/constants.js';
import { getMeas } from '../lib/helpers.js';
...
function ReviewPinRow({ pin, canApprove, showActions, onApprove, onDecline, onPhotoNav }) {
  ...
  const photoIds = pin.surveyPhotoIds || [];
  const legacyPhotos = pin.surveyPhotos || [];
    ...
    <div style={{ padding: '14px 16px', background: isDecl ? '#fef2f2' : isAppr ? '#f0fdf4' : C.card, border: '1px solid ' + (isDecl ? '#fecaca' : isAppr ? '#bbf7d0' : C.border), borderRadius: 10, marginBottom: 8 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {(photoIds.length > 0 || legacyPhotos.length > 0) && (
            <div style={{ display: 'flex', gap: 4, marginTop: 8 }}>
              {photoIds.map((id, i) => (
                <div key={id} onClick={() => onPhotoNav({ ids: photoIds, startIdx: i })} style={{ width: 48, height: 36, borderRadius: 4, overflow: 'hidden', border: '1px solid ' + C.border, cursor: 'pointer' }}>
                  <PhotoImg photoId={id} style={{ width: '100%', height: '100%', objectFit: 'cover' }}/>
                </div>
              ))}
              {photoIds.length === 0 && legacyPhotos.map((p, i) => (
                <div key={i} onClick={() => onPhotoNav({ urls: legacyPhotos, startIdx: i })} style={{ width: 48, height: 36, borderRadius: 4, overflow: 'hidden', border: '1px solid ' + C.border, cursor: 'pointer' }}>
