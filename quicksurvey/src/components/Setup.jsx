...
import { INPUT_STYLE as IS, LABEL_STYLE as LS } from './UserSetup.jsx';
...
export function Setup({ onDone, onBack }) {
  ...
  const updCnt = (n) => {
    const c = Math.min(8, Math.max(1, parseInt(n) || 1));
    setCnt(c);
    setNames(p => {
      const a = [...p];
      while (a.length < c) a.push(defs[a.length] || 'Elevation ' + (a.length + 1));
      return a.slice(0, c);
    });
  };
