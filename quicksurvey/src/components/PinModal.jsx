...
import { pCol, getMeas } from '../lib/helpers.js';
import { processAndStorePhoto, compress } from '../lib/photo.js';
import { PhotoImg } from './PhotoImg.jsx';
import { PhotoNav } from './PhotoNav.jsx';

// Drawing canvas only loads when the user actually wants to annotate
const DrawingCanvas = lazy(() => import('./DrawingCanvas.jsx').then(m => ({ default: m.DrawingCanvas })));
...
export function PinModal({ pin, repairList, onSave, onTrash, onDuplicate, onClose, onAddRT, isMobile, user, projectId }) {
  ...
  // Staging area: array of { blob, drawnBlob? } — original captured Blobs, edited if user drew on them
  ...
  // Photo IDs from saved pin (new format) — falls back to old base64 arrays for legacy pins
  ...
  // Legacy support: very old pins still have base64 arrays
  const legacyPhotos = [...(pin.surveyPhotos || []), ...(pin.fixingPhotos || []), ...(pin.donePhotos || [])];
  ...
  // Add files to staging. We keep them as Blobs so compression/watermark happens at save time.
  const addPhotos = files => {
    const rem = MAX - staging.length;
    const slice = Array.from(files).slice(0, rem);
    setStaging(p => [...p, ...slice.map(f => ({ blob: f }))]);
  };

  const pickR = r => { setSelR(r); setRtype(r.type); setMeas({}); setView('fill'); };

  // Save survey (creating a new pin)
  const saveSurvey = async () => {
    ...
    // Process each staged photo: watermark and store as Blob, returning a photoId
    const photoIds = [];
    for (const item of staging) {
      const blob = item.blob;
      const id = await processAndStorePhoto(blob, pinData, user.name, projectId);
      photoIds.push(id);
    }
  ...
  const savePhase = async (type) => {
    ...
    for (const item of staging) {
      const blob = item.blob;
      const id = await processAndStorePhoto(blob, pinForWm, user.name, projectId);
      photoIds.push(id);
    }

    let updated = { ...pin, measurements: { ...meas } };
    if (type === 'fixing') {
      updated.fixingPhotoIds = photoIds;
      updated.status = ST.FIXING;
      if (comment.trim()) updated.fixingComment = comment.trim();
    } else {
      updated.donePhotoIds = photoIds;
      updated.status = ST.DONE;
      if (comment.trim()) updated.doneComment = comment.trim();
    }
    onSave(updated);
    setSaving(false);
  };

  // Local URL preview for staging items — uses createObjectURL on the fly
  const StagedThumb = ({ item, onDraw, onRemove }) => {
    const [url, setUrl] = useState(null);
    if (!url && item.blob) {
      // Generate a one-shot URL — note we don't revoke it (component will unmount soon enough)
      const u = URL.createObjectURL(item.blob);
      setUrl(u);
    }
    return (
      <div style={{ position: 'relative', width: 60, height: 48, borderRadius: 6, overflow: 'hidden', border: '1px solid ' + C.border }}>
        {url && <img src={url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt=""/>}
        <button onClick={onDraw} style={{ position: 'absolute', bottom: 1, right: 1, background: '#2563eb99', border: 'none', borderRadius: '50%', color: '#fff', width: 18, height: 18, cursor: 'pointer', fontSize: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }} title="Draw on photo">✎</button>
        <button onClick={onRemove} style={{ position: 'absolute', top: 1, right: 1, background: 'rgba(220,38,38,0.85)', border: 'none', borderRadius: '50%', color: '#fff', width: 16, height: 16, cursor: 'pointer', fontSize: 9, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
      </div>
    );
  };
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
          {staging.map((item, i) => (
            <StagedThumb
              key={i}
              item={item}
              onDraw={() => setDrawingMode(i)}
              onRemove={() => setStaging(s => s.filter((_, j) => j !== i))}/>
          ))}
        </div>
      )}
      {staging.length < MAX && (
  ...
  if (drawingMode !== null) {
    const item = staging[drawingMode];
    return (
        ...
        <DrawingCanvas
          ...
          onClose={drawnBlob => {
            if (drawnBlob) {
              setStaging(s => {
                const ns = [...s];
                ns[drawingMode] = { ...ns[drawingMode], blob: drawnBlob };
                return ns;
              });
            }
            setDrawingMode(null);
        ...
        <div style={{ padding: '16px 17px' }}>
          {view === 'status' && (
            <>
              ...
              {(allIds.length > 0 || legacyPhotos.length > 0) && (
                <div style={{ marginBottom: 14 }}>
                  <div style={{ fontSize: 9, color: C.textMuted, marginBottom: 6, letterSpacing: 1.5, fontFamily: 'Barlow Condensed', fontWeight: 700 }}>PHOTOS ({allIds.length || legacyPhotos.length})</div>
                  <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                    {allIds.map((id, i) => (
                      <div key={id} onClick={() => setPhotoNav({ ids: allIds, startIdx: i })} style={{ width: 52, height: 40, borderRadius: 5, overflow: 'hidden', border: '1px solid ' + C.border, cursor: 'pointer' }}>
                        <PhotoImg photoId={id} style={{ width: '100%', height: '100%', objectFit: 'cover' }}/>
                      </div>
                    ))}
                    {/* Legacy fallback */}
                    {allIds.length === 0 && legacyPhotos.map((p, i) => (
                      <div key={i} onClick={() => setPhotoNav({ urls: legacyPhotos, startIdx: i })} style={{ width: 52, height: 40, borderRadius: 5, overflow: 'hidden', border: '1px solid ' + C.border, cursor: 'pointer' }}>
          ...
          {(view === 'fixing' || view === 'done') && (() => {
            const isFixing = view === 'fixing';
            const phaseColor = isFixing ? '#7c3aed' : '#16a34a';
            const phaseBg = isFixing ? '#8b5cf610' : '#22c55e10';
            const phaseBorder = isFixing ? '#8b5cf633' : '#22c55e33';
            const phaseLabel = isFixing ? (isDeclined ? 'FIXING DECLINED REPAIR' : 'FIXING PHOTOS') : (isDeclined ? 'FINISHING DECLINED REPAIR' : 'DONE PHOTOS');
            return (
              <>
                <div style={{ padding: '10px 14px', background: isDeclined ? '#fef2f2' : phaseBg, borderRadius: 8, border: '1px solid ' + (isDeclined ? '#fecaca' : phaseBorder), marginBottom: 14 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: isDeclined ? C.declined : phaseColor, fontFamily: 'Barlow Condensed' }}>{phaseLabel}</div>
                  <div style={{ fontSize: 11, color: isDeclined ? '#dc262699' : phaseColor + '99', marginTop: 2 }}>1–3 photos{!isOther && isFixing ? ' · you can update the measurement if needed' : isFixing ? '' : ' · document the completed repair'}</div>
                </div>
                {rt && !isOther && (
                  <>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: rt.fields.length === 3 ? '1fr 1fr 1fr' : '1fr 1fr', gap: 6, marginBottom: 6 }}>
                      {rt.fields.map(f => (
                        <div key={f.k}>
                          <div style={{ fontSize: 9, color: C.textMuted, marginBottom: 3 }}>{f.l}</div>
                          <input type="number" min="0" value={meas[f.k] || ''} onChange={e => { const v = e.target.value; setMeas(prev => ({ ...prev, [f.k]: v })); }} placeholder="0" style={{ ...IS, padding: '7px 8px', borderColor: measChanged ? phaseColor : C.border }}/>
                        </div>
                      ))}
                    </div>
                ...
                <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder={measChanged ? 'Explain why the measurement changed...' : (isFixing ? 'Notes for this phase...' : 'Final notes...')} rows={2} style={{ ...IS, resize: 'none', marginBottom: 4, borderColor: measChanged && comment.trim().length < 10 ? '#dc2626' : C.border }}/>
                ...
                <label style={LS}>{isFixing ? 'FIXING' : 'DONE'} PHOTOS (1–3)</label>
                <StagingArea label={`Add ${isFixing ? 'fixing' : 'done'} photos`}/>
                <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                  ...
                  <button onClick={() => savePhase(view)} disabled={saving || !canSave || !commentOk}
                    style={{ flex: 2, padding: '10px', borderRadius: 8, border: 'none', background: (canSave && commentOk) ? (isDeclined ? '#dc2626' : phaseColor) : '#e5e7eb', color: (canSave && commentOk) ? '#fff' : '#9ca3af', fontWeight: 700, fontSize: 13, cursor: 'pointer', opacity: saving ? 0.6 : 1, fontFamily: 'Barlow Condensed' }}>
                    {saving ? 'SAVING...' : isFixing ? 'SAVE FIXING →' : (isDeclined ? 'CLOSE DECLINED ✓' : 'MARK DONE ✓')}
                  </button>
                </div>
              </>
            );
