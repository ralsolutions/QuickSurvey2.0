...
import { C, ROLES, ST, dbGetAll, dbDelete, dbPut, exportProject, importProjectFromFile } from '../lib/shared.js';
import { RoleSwitcher } from './RoleSwitcher.jsx';
import { ElevationsManager } from './ElevationsManager.jsx';
...
export function HomeScreen({ onOpen, onCreate, user, onRoleSwitch, onSwitchUser, onLogout }) {
  ...
  const applyElevations = async (elevs) => {
    const np = { ...editElev, elevations: elevs };
    await dbPut(np);
    setList(l => l.map(x => (x.id === np.id ? np : x)));
    setEditElev(null);
  };
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: C.bg }}>
          ...
          {list.map(p => {
            ...
            return (
              <div key={p.id} style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 10, padding: '14px 16px', display: 'flex', alignItems: 'center', cursor: 'pointer', marginBottom: 8, boxShadow: '0 1px 3px rgba(0,0,0,0.06)', transition: 'box-shadow 0.15s' }}
                ...
                {canCreate && <button title="Manage elevations" style={{ background: 'none', border: 'none', color: C.textMuted, cursor: 'pointer', fontSize: 13, padding: '4px 8px' }} onClick={e => { e.stopPropagation(); setEditElev(p); }}>⚙</button>}
      ...
      {editElev && <ElevationsManager elevations={editElev.elevations} title={'Elevations — ' + editElev.name} onApply={applyElevations} onClose={() => setEditElev(null)}/>}
    </div>
  );
}
