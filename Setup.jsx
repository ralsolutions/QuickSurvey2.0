...
import { C, RT, HC, HAZARDS, ST, SC, SL, IS, LS, pCol, getMeas, wm, cleanNumeric, plabel } from '../lib/shared.js';
import { DrawingCanvas } from './DrawingCanvas.jsx';
import { PhotoNav } from './PhotoNav.jsx';

export function PinModal({ pin, project, repairList, onSave, onTrash, onDuplicate, onClose, onAddRT, isMobile, user }) {
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: C.card, border: '1px solid ' + (isDeclined ? C.declined + '55' : isNew ? C.border : hbc + '55'), borderRadius: isMobile ? '16px 16px 0 0' : 14, width: isMobile ? '100%' : 390, maxWidth: '100%', maxHeight: isMobile ? '94vh' : '92vh', overflowY: 'auto', boxShadow: '0 -10px 40px rgba(0,0,0,0.25)', paddingBottom: isMobile ? 'env(safe-area-inset-bottom)' : 0 }}>
        ...
        <div style={{ padding: '13px 17px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: isDeclined ? '#450a0a' : C.navyDark, borderRadius: isMobile ? '16px 16px 0 0' : '14px 14px 0 0', position: 'sticky', top: 0, zIndex: 2 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            ...
            {!isNew && <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: statusColor + '33', color: statusColor, letterSpacing: 1, border: '1px solid ' + statusColor + '55' }}>{isDeclined ? 'DECLINED' : plabel(project, cur)}</span>}
          </div>
                ...
                {[ST.TOREPAIR, ST.FIXING, ST.DONE].map((s, i) => {
                  ...
                  return (
                    <div key={s} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
                      ...
                      <span style={{ fontSize: 8, color: col, fontFamily: 'Barlow Condensed', fontWeight: reached ? 700 : 400 }}>{plabel(project, s).toUpperCase()}</span>
                    </div>
                  );
                })}
              </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: rt.fields.length === 3 ? '1fr 1fr 1fr' : '1fr 1fr', gap: 6, marginBottom: 6 }}>
                    {rt.fields.map(f => (
                      <div key={f.k}>
                        ...
                        <input type="text" inputMode="decimal" value={meas[f.k] || ''} onChange={e => { const v = cleanNumeric(e.target.value); setMeas(prev => ({ ...prev, [f.k]: v })); }} placeholder="0" style={{ ...IS, padding: '7px 8px' }}/>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: rt.fields.length === 3 ? '1fr 1fr 1fr' : '1fr 1fr', gap: 6, marginBottom: 6 }}>
                    {rt.fields.map(f => (
                      <div key={f.k}><div style={{ fontSize: 9, color: C.textMuted, marginBottom: 3 }}>{f.l}</div>
                        <input type="text" inputMode="decimal" value={meas[f.k] || ''} onChange={e => { const v = cleanNumeric(e.target.value); setMeas(prev => ({ ...prev, [f.k]: v })); }} placeholder="0" style={{ ...IS, padding: '7px 8px', borderColor: measChanged ? '#8b5cf6' : C.border }}/></div>
                    ))}
                  </div>
              ...
              {rt && !isOther && (
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: rt.fields.length === 3 ? '1fr 1fr 1fr' : '1fr 1fr', gap: 6, marginBottom: 6 }}>
                    {rt.fields.map(f => (
                      <div key={f.k}><div style={{ fontSize: 9, color: C.textMuted, marginBottom: 3 }}>{f.l}</div>
                        <input type="text" inputMode="decimal" value={meas[f.k] || ''} onChange={e => { const v = cleanNumeric(e.target.value); setMeas(prev => ({ ...prev, [f.k]: v })); }} placeholder="0" style={{ ...IS, padding: '7px 8px', borderColor: measChanged ? '#22c55e' : C.border }}/></div>
                    ))}
                  </div>
                  <div style={{ fontSize: 12, color: C.blue, marginBottom: 12, fontFamily: 'DM Mono', padding: '5px 10px', background: C.blueDim, borderRadius: 6, border: '1px solid ' + C.blueBorder }}>= {rt.calc(meas)}</div>
                </>
