...
import { C, RT, HC, HAZARDS, ST, SC, SL, useIsMobile, getMeas, dbGetPrices, dbSavePrices } from '../lib/shared.js';
...
export function SummaryTable({ project, onClose, user }) {
    ...
    return (
      <div style={{ background: isDecl ? '#fef2f2' : C.card, border: '1px solid ' + (isDecl ? '#fecaca' : C.border), borderRadius: 10, padding: '12px 14px', marginBottom: 8, boxShadow: '0 1px 2px rgba(0,0,0,0.04)' }}>
        ...
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: allP.length > 0 || pin.comment || pin.fixingComment || pin.doneComment || pin.approvalComment ? 8 : 0 }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: sc, background: sc + '18', padding: '3px 8px', borderRadius: 6, fontFamily: 'Barlow Condensed', letterSpacing: 0.3 }}>{SL[pin.status] || '—'}</span>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 250, padding: isMobile ? 0 : 16 }}>
      <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: isMobile ? 0 : 14, width: '100%', maxWidth: isMobile ? '100%' : 1060, maxHeight: isMobile ? '100vh' : '92vh', height: isMobile ? '100vh' : 'auto', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
        <div style={{ padding: isMobile ? '12px 14px' : '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0, background: C.navyDark, borderRadius: isMobile ? 0 : '14px 14px 0 0', paddingTop: isMobile ? 'calc(12px + env(safe-area-inset-top))' : undefined }}>
          <div>
